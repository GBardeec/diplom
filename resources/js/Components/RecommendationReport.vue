<template>
  ...
  <template v-else>
    ...
    <article v-if="directions.length" class="panel">
      ...
      <p class="summary-text">Основной вариант выбран по совпадению навыков и достаточному числу вакансий. Грейд пользователя и рыночный уровень роли считаются отдельно.</p>
      ...
      <section v-if="activeDirection" class="direction mt-4">
        ...
        <p class="mt-1 text-sm text-[#616161]">Роли ниже подходят по выбранным навыкам. Рыночный уровень показывает положение роли по зарплатам внутри направления, а не ваш личный грейд.</p>
        <div class="mt-4"><p class="subheading">Подходящие роли</p><div class="mt-2 grid gap-3 sm:grid-cols-3"><button v-for="role in activeDirection.roles" :key="role.category_id" type="button" class="inner role-card" :class="{ active: activeRole?.category_id === role.category_id }" @click="activeRoleId = role.category_id"><b>{{ role.title }}</b><p>{{ vacancyLabel(role.vacancies_count) }} в выборке</p><p v-if="role.market_level" class="mt-1 text-xs text-[#6d7175]">Рыночный уровень {{ role.market_level }}<span v-if="role.market_salary_median"> - медиана {{ formatSalary(role.market_salary_median) }}</span></p><strong>{{ role.matched_skills_count }} из {{ role.selected_skills_count }} выбранных навыков встречаются в роли</strong></button></div></div>
        <div v-if="activeRole" class="mt-4"><p class="subheading">Навыки роли: {{ activeRole.title }}</p><p class="mt-1 text-sm text-[#616161]">Проценты показывают, в какой доле вакансий этой роли встречается навык.</p><div v-if="activeRole.skills?.length" class="mt-2 grid gap-2 sm:grid-cols-2"><div v-for="skill in activeRole.skills" :key="skill.id" class="skill-gap"><b>{{ skill.title }}</b><span>{{ skill.percent }}% вакансий</span></div></div><p v-else class="mt-2 text-sm text-[#616161]">По этой роли пока нет данных о навыках.</p><div class="mt-4 grid gap-4 lg:grid-cols-2"><div class="inner"><p class="subheading">Приоритет навыков</p><p class="mt-2 text-sm text-[#616161]"><b>Основной дефицит:</b> {{ primaryGaps.join(', ') || 'не выявлен' }}.</p><p class="mt-2 text-sm text-[#616161]"><b>Полезно усилить:</b> {{ strengthsToBuild.join(', ') || 'нет частых выбранных навыков' }}.</p><p class="mt-2 text-sm text-[#616161]"><b>Дополнительный стек:</b> {{ optionalSkills.join(', ') || 'не выявлен' }}.</p></div><div class="inner"><p class="subheading">Данные рынка роли</p><p class="mt-2 text-sm text-[#616161]">{{ vacancyLabel(activeRole.vacancies_count) }} в анализе. Медиана зарплаты: {{ activeRole.market_salary_median ? formatSalary(activeRole.market_salary_median) : 'нет данных' }}.</p><p class="mt-2 text-sm text-[#616161]">Города: {{ activeRole.locations?.map(location => `${location.title} (${location.percent}%)`).join(', ') || 'нет данных' }}.</p><a :href="`/hierarchy-structure?group=${activeDirection.group_id}&role=${activeRole.category_id}`" class="market-link">Посмотреть роль на карте уровней</a></div></div><div v-if="comparisonOptions.length" class="mt-4"><label class="subheading" for="role-comparison">Сравнить с другой ролью</label><select id="role-comparison" v-model="comparisonRoleId" class="comparison-select"><option :value="null">Не сравнивать</option><option v-for="role in comparisonOptions" :key="role.category_id" :value="role.category_id">{{ role.title }}</option></select><div v-if="comparisonRole" class="mt-3 grid gap-3 sm:grid-cols-2"><div class="inner"><p class="subheading">Общие навыки</p><p class="mt-2 text-sm text-[#616161]">{{ commonSkills.join(', ') || 'нет в верхних навыках выборки' }}.</p></div><div class="inner"><p class="subheading">Отличаются</p><p class="mt-2 text-sm text-[#616161]">Только у {{ activeRole.title }}: {{ onlyActiveSkills.join(', ') || 'нет' }}.</p><p class="mt-2 text-sm text-[#616161]">Только у {{ comparisonRole.title }}: {{ onlyComparisonSkills.join(', ') || 'нет' }}.</p></div></div></div></div>
