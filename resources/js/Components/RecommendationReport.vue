<template>
  ...
  <template v-else>
    ...
    <article v-if="result.readiness" class="panel"><div class="flex flex-wrap items-start justify-between gap-3"><div><h2>Готовность к следующему шагу</h2><p class="summary-text">{{ result.readiness.summary }}</p></div><span class="readiness">{{ result.readiness.status }}</span></div></article>
    ...
    <article v-if="result.portfolio_evidence" class="panel"><h2>Что показать работодателю</h2><div class="mt-4 grid gap-3 sm:grid-cols-3"><div v-for="(item, index) in result.portfolio_evidence" :key="item.title" class="inner"><span class="number">{{ index + 1 }}</span><b>{{ item.title }}</b><p>{{ item.text }}</p></div></div></article>
