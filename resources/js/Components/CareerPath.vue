<template>
    <div class="career-path">
        ...
        <div class="career-steps" :style="{ gridTemplateColumns: `repeat(${nodes.length}, minmax(0, 1fr))` }">
            <div v-for="node in nodes" :key="node.id" class="career-step"><span class="career-dot"></span><span class="career-title">{{ node.title }}</span><small>Уровень {{ node.market_level }}</small></div>
        </div>
    </div>
</template>
...
<style scoped>
.career-path { position:relative; overflow-x:auto; padding:1rem .5rem .25rem; }.career-lines { position:absolute; top:1rem; left:.5rem; width:calc(100% - 1rem); height:32px; }.career-steps { position:relative; display:grid; min-width:420px; }.career-step { display:flex; flex-direction:column; align-items:center; gap:.45rem; text-align:center; color:#4a4f54; font-size:.85rem; }.career-step small { color:#6d7175; font-size:.72rem; }.career-dot { z-index:1; width:.9rem; height:.9rem; border:3px solid #fff; border-radius:50%; background:#008060; box-shadow:0 0 0 1px #008060; }.career-title { max-width:10rem; line-height:1.25; }
</style>
